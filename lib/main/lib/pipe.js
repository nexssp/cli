...
async function run(operations, options = {}) {
  // console.log('run at:', yellow(bold(process.cwd())))
  if (cliArgs.nxsTime) {
  ...
  const finalOperations = operations.map((element, index) => {
    ...
    runOptions.pureArgs = element.pureArgs
    ...
    if (element.cmd) {
      if (element.path) {
        runOptions.path = element.path
      }

      // if (streamName === 'transformFile') {
      //   // runOptions.cwd = process.cwd()
      //   delete runOptions.env
      //   console.log(element)
      //   console.log(cmd, args, runOptions, process.cwd())
      //   process.exit(1)
      // }

      return eval(streamName)(cmd, args, runOptions)
    }
