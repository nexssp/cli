...
module.exports.transformOutput = (cmd, currentArgs, options) => {
  ...
  return new Transform({
    ...
    transform: (chunk, encoding, callback) => {
      ...
      const cliArgs = require('minimist')(currentArgs)
      delete cliArgs._
      ...
      if (data !== undefined) {
        ...
        if (data.nxsOutCombine) {
          const nxsCombine = require('./output/nxsCombine')
          data = nxsCombine(data)
        }
        ...
        if (cliArgs.nxsDataSize) {
          const { dataSize } = require('./output/nxsDataSize')
          dataSize(data)
        }
