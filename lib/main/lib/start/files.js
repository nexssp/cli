...
const getFiles = (folder, args, env, ccc, pureArguments) => {
  fdebug('Starting..')
  ...
  // ====================================================================
  // Command is a file so we return here with the file info and arguments
  // ====================================================================
  ...
  const resultFiles =
    ...
    config_files.map((file) => {
      ...
      if (fs.lstatSync(ppp).isDirectory()) {
        ...
        const xxxx = getFiles(file, file.data, env, subConfig, pureArguments)
        return xxxx
      }
      // } else {
