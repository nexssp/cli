...
module.exports.parseError = (filename, errorBody, stdOutput, cwd, isErrorPiped) => {
  if (errorBody && errorBody.trim) {
    errorBody = errorBody.trim()
  }

  cwd = cwd || process.nexssCWD

  const { language1 } = require('../../config/language')
  language1.start()

  const langInfo = language1.byFilename(filename)
  ...
  const nexssConfigGlobal = config1.load(`${process.NexssFilePath}/_nexss.yml`)
  const nexssConfigLocal = config1.load(`${cwd}/_nexss.yml`)

  // We load in the order language errors, global errors (where the file exists - it can be package),
  // and then current folder
  langInfo.errors = {
    ...langInfo.errors,
    ...(nexssConfigGlobal && nexssConfigGlobal.errors),
    ...(nexssConfigLocal && nexssConfigLocal.errors),
  }
  ...
  if (langInfo.errors) {
    Object.keys(langInfo.errors).forEach((pattern) => {
      ...
      try {
        ...
        log.error('Check error definition in the directory:', dirname(langInfo.configFile))
        process.exit(0)
      }
      let match
      ...
      if (solution && solution.includes && !solution.includes(`${filename}.exe`)) {
        if (stdOutput) {
          if (cliArgs.htmlOutput) {
            console.log('<BR/>')
            console.log(
              `${`<span style="color:green"><BR /><b>Possible solution${solutionNumber}</B>`}: ${solution}</span>`
            )
          } else if (isErrorPiped) {
          ...
          console.log(yellow(`Possible solution ${solutionNumber}: ${bold(yellow(solution))}`))
        } else {
          log.error(yellow(`Possible solution ${solutionNumber}: ${bold(yellow(solution))}`))
        }
        solutionNumber++
      }
    })
