...
module.exports.readable = (startData) => {
  ...
  if (cliArgs.nxsTest) {
    ...
    const testDataPassed = cliArgs.testData || cliArgs.testdata
    if (!testDataPassed) {
      testData = require('../../config/testingData.json')
    } else {
      const testDataPath = path.normalize(`${process.cwd()}/${testDataPassed}`)
      try {
        testData = require(testDataPath)
      } catch (_) {
        if (!fs.existsSync(testDataPath)) {
          log.error(
            `Your test data ${bold(testDataPath)} must be valid json or JavaScript/NodeJS file!`
          )
        } else {
          log.error(
            `Your test data ${bold(testDataPath)} must be valid json or JavaScript/NodeJS file!`
  ...
  // }
  if (cliArgs._) {
    // if (['.', 'start', 's'].includes(cliArgs._[0])) {
    //   cliArgs._.shift()
    // }

    if (!startWithSpecialChar(process.argv[2])) {
      cliArgs._.shift()
    }
  ...
  if (process.NexssFilePath && process.NexssFilePath !== '.') {
    ...
    if (!startData.__dirname) startData.__dirname = process.NexssFilePath

    // Below is constant (maybe later to modify if nexss files are not working???)

    const nexssFileStartData = { ...startData }

    nexssFileStartData.cwd = process.cwd()
