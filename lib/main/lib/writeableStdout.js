  ...
  return new Writable({
    ...
    write: (chunk, encoding, callback) => {
      if (chunk.stream === 'cancel') {
        ...
        if (!outputPretty) {
          if (chunk[outputKeys]) {
            const color = '\u001b[35m' // gray

            console.error(`${color}Display below only Object keys (${outputKeys}):\x1b[0m`)
            console.error(JSON.stringify(Object.keys(chunk)))
          } else {
            chunk =
              typeof chunk === 'object' || typeof chunk === 'number' ? JSON.stringify(chunk) : chunk

            if (is('json', chunk)) {
              let outputJson = `${chunk}`

              if (
                process.nexssGlobalConfig[nexss['output:colors']] ||
          ...
          if (
            (process.nexssGlobalConfig.colors && process.nexssGlobalConfig.colors.output) ||
            cliArgs[nexss['output:colors']]
          ) {
