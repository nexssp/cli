...
const { red, bold, yellow, cyan, cyanBG } = require('@nexssp/ansi')
...
module.exports.worker = function ({
  ...
  if (!callback) {
    ...
    throw new Error(`Callback needs to be a function. But received: ${typeof callback}`)
  }
  ...
  const dataOnError = [{ chunk_in: chunk }] // just to display if end with error. Think about better way.
  const dataOnErrorCWD = chunk.data.cwd
  ...
  const allArguments = `${argsStrings.join(' ').trim()} ${pureArgs.join(' ').trim()}`
  log.dy(`Spawning ..${cmd}`, 'with args ', argsStrings.join(' '), 'cwd: ', process.cwd())
  const Worker1 = spawn(cmd, argsStrings.concat(pureArgs), options)

  const nexssCommand = `${cmd} ${allArguments}`
  ...
  Worker1.on &&
    Worker1.on('error', (err) => {
      ...
      switch (err.code) {
        case 'ENOENT':
          log.error(
            `TRANSFORM_nexss:${err.path} not found. Command: ${cmd} ${argsStrings.join(' ')}`
          )
          break
        default:
          log.error(
      ...
      if (errorString.includes('NEXSS/')) {
        ...
        exploded.forEach((element) => {
          ...
          if (element.substring(0, 1) === '/') {
            ...
            parseError(fileName, element, argsStrings.includes('--nxsPipeErrors'), process.nexssCWD)
          }
        })
        // this.errBuffer = "";
      } else {
        ...
        if (this.errBuffer)
          parseError(fileName, this.errBuffer, argsStrings.includes('--nxsPipeErrors'))
        this.errBuffer = ''
      }
    })
  ...
  Worker1.stdout &&
    ...
    Worker1.stdout.on('data', (data) => {
      data = data.toString()
      const optionsWithoutEnv = { ...options, env: 'hidden' }
      dataOnError.push({
  ...
  Worker1.stderr.on('end', function () {
    ...
    if (this.errBuffer) {
      parseError(fileName, this.errBuffer, argsStrings.includes('--nxsPipeErrors'), process.cwd())
  ...
  Worker1.on('exit', (code) => {
    if (process.argv.includes('--debug') && code !== 0 && dataOnError.length > 1) {
      ...
      log.error(
        `\n${bold(yellow(Worker1.cmd))} has ended with exitCode ${yellow(bold(code))}. \n${bold(
          `Nexss Programmer will not continue.`
        )}`
      )
      const sliced = dataOnError.slice(1, -1)
      console.log(
        require('util').inspect(sliced, {
          compact: true,
          depth: 5,
          breakLength: 80,
          colors: true,
      ...
      console.log(cyanBG('OUTPUT:'))
      console.log(sliced[0].data_from_worker)
    }
