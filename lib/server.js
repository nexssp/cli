const { yellow, bold } = require("./ansi");
    ...
    async function (req, res) {
      ...
      nexssStartArgs = nexssStartArgs.filter((e) => {
        return (
          ...
          !e.startsWith("--nxsUrlSearch=")
        ); // && !e.startsWith("--nxsField=nxsOut");
      });
      ...
      const args = [
        path.resolve(process.env.NEXSS_SRC_PATH, "nexss.js"),
        ...nexssStartArgs,
      ];

      console.log("Spawning: ", node, args.join(" "));

      var responder = spawn(node, args, spawnOptions);
