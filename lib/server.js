...
module.exports.startServer = (server, opts, runSequence) => {
  ...
  var server = http.createServer(
    ...
    async function (req, res) {
      ...
      // console.log(`url parsed: ${require("util").inspect(url)}`);
      ...
      nexssStartArgs = nexssStartArgs.filter((e) => {
        return !e.startsWith("--seq=") && !e.startsWith("--nxsUrlPath="); // && !e.startsWith("--nxsField=nxsOut");
      });

      nexssStartArgs = nexssStartArgs.concat(`--nxsUrlPath=${url.pathname}`);
      ...
      if (url.pathname && url.pathname !== "/") {
        ...
        if (extension in mimeTypes) {
          ...
          if (!fs.existsSync(url.pathname)) {
            ...
            res.end(`File not found: ${url.pathname}, ${process.cwd()}`);
            console.timeEnd("Nexss request");
            return;
          }
  ...
  server.listen(port, host, () => {
    console.log(
      `Your Nexss Programmer server is running at http://${hostAddress}/`
    );
  });
