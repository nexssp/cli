module.exports = (nexssArgs) => {
  ...
  nConst('aliases', require('./aliases'), process)
  ...
  nConst('cliArgs', require('minimist')(nexssArgs))
  ...
  const globalConfigPath = path.normalize(`${require('os').homedir()}/.nexss/config.json`)
  ...
  if (nxsPlatform && nxsPlatform.split) {
    ...
    if (!checkPlatform(platforms)) {
      const info = `${yellow('Nexss Programmer: ')}${bold(
        red(platforms.join(', '))
      )} did not match with your platform ${green(bold(process.platform))}, ${green(
        bold(process.distroTag1)
      )} or ${green(bold(process.distroTag2))}. Program will NOT continue.`

      if (!cliArgs[nexss['platform:noerror']]) {
        console.error(info)
        process.exit(1)
      } else {
        log.warn(`WARN: `, info.replace('Program will NOT continue', 'But program WILL continue'))
        process.exit(0)
      }
    }
  }
