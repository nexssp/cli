...
module.exports.startServer = (server, opts, runSequence) => {
  ...
  const index = nexssStartArgs.findIndex(element => element.startsWith("--server"));
  if (index > -1) {
  ...
  // To update as reference
  const portRef = { port }

  server.listen(port, host, () => {
    console.log(
      `Your Nexss Programmer server is running. ${green(bold(host + ':' + portRef.port))}`
    )
  })
  ...
  server.on('error', (e) => {
    if (e.code === 'EADDRINUSE') {
      console.log(
        `Address in use: ${red(bold(e.address + ':' + e.port))},\nRetrying with port: ${yellow(
          bold(e.address + ':' + ++portRef.port)
        )}`
      )
      setTimeout(() => {
        server.close()
