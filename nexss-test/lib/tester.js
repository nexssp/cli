module.exports = function testNexss(testNames) {
  ...
  const availTests = (testsPath, nexssConfig) => {
  ...
  if (cliArgs._[0] !== "all") {
    ...
    if (!fs.existsSync(`${nexssTestsFolder}/${oneTest}`)) {
      ...
      availTests(nexssTestsFolder, nexssConfig);
      process.exit();
    }
    testNames = [oneTest];
  } else {
  ...
  function should(fname, test, regE, options) {
    ...
    if (test == "null") {
      ...
      if (!process.testData) {
        console.error(
          "You need to specify REGEXP or STRING for the first test."
        );
        process.exit();
      }
      out(
        grey(
          `Using cached result of previous command: ${bold(process.testTest)}`
        )
      );
      ...
      data = exe(test);
      console.log("XXXXXX", data, "YYYYYYYYYYYY");
      data = process.testData = data.replace(
