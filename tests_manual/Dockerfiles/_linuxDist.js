...
switch (process.argv[3]) {
  ...
  case "local-testlangs":
    command = `docker run ${privileged} -i ${detached} -v ${pathApps}:/root/.nexssApps -v ${pathWork}:/work -v ${pathNexssCli}:/nexssCli -v ${pathDotNexss}:/root/.nexss -v /root/.nexss/cache -e DEBIAN_FRONTEND=noninteractive -t ${imageName} ${shell} -c "cd /nexssCli/src && chmod +x nexss.js && ln -s $(pwd)/nexss.js /usr/bin/nexss && cd /work && nexss test languages && ${shell}" `;
    break;
  case "local-empty":
    command = `docker run ${privileged} -i ${detached} -v /work -v ${pathNexssCli}:/nexssCli -v /root/.nexss/cache -e DEBIAN_FRONTEND=noninteractive -t ${imageName} ${shell} -c "cd /nexssCli/src && chmod +x nexss.js && ln -s $(pwd)/nexss.js /usr/bin/nexss && cd /work && ${shell}" `;
    break;
  case "clone":
